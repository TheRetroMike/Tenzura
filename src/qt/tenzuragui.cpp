...
void TenzuraGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Tenzura address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and tenzura: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to TENZ addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Tenzura addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Tenzura addresses"));
    ...
    openAction->setStatusTip(tr("Open a tenzura: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Tenzura command-line options").arg(tr(PACKAGE_NAME)));
...
//        m_toolbar->addAction(votingAction);
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/Tenzura/Tenzura/releases\">New Wallet Version Available</a>");
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/Tenzura/Tenzura/releases";
...
void TenzuraGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Tenzura network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void TenzuraGUI::checkAssets()
{
    ...
    if(AreAssetsDeployed()) {
        ...
        transferAssetAction->setToolTip(tr("Transfer assets to TENZ addresses"));
...
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/Tenzura/Tenzura/releases"));
