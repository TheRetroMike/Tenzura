...
====================
Some notes on how to build Tenzura Core in *nix.


Note
---------------------
Always use absolute paths to configure and compile tenzura and the dependencies,
...
This will build tenzura-qt as well if the dependencies are met.
...
Security
--------
To help make your tenzura installation more secure by making certain attacks impossible to
...
* Non-executable Stack
    ...
    vulnerable buffers are found. By default, tenzura should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, tenzura may be compiled in
...
`--with-incompatible-bdb` according to the [PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/tenzura/trunk/PKGBUILD).
As mentioned above, when maintaining portability of the wallet between the standard Tenzura Core distributions and independently built
