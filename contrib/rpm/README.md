...
The RPM spec file provided here is for Tenzura-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://www.tenzura.io/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Tenzura git repository](https://github.com/Tenzura/Tenzura) but are not part of
...
Currently only `Source10` is used, for the example `tenzura.conf` file.
...
in the tenzura git repository. At present only an SVG version of the Tenzura
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the tenzura
...
needed when 0.12.1 is released, a proper fix is already in the Tenzura git
master branch.
...
idea to build Tenzura Core against a static version of that library compiled
...
The `tenzura.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `tenzura-server` package. At this point, most RPM based
...
The `tenzura-server` package should have SELinux support. How to properly do
...
Tenzura-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/tenzura/). He has been
packaging Tenzura for Fedora at least since 2012.
...
4. I renamed the `tenzura` package that contains the Qt GUI to `tenzura-core` as
that appears to be how the general population refers to it, in contrast to
`tenzura-xt` or `tenzura-classic`. I wanted to make sure the general population
